...
git-drs is built on git, so many commands behave similarly to git with some key differences. These differences will be outlined for each step in the submission process.
...
!!! note
    The following examples will use the `calypr` program with a project called `myproject` and an `calypr` git-drs profile.
...
For most git-drs commands, you need to specify the gen3-client profile you want to use. This ensures that you are uploading projects to the right platform with the right credentials. There are two ways to set your profile...
...
To initialize your new project locally, you can use `git-drs init`

```bash
mkdir calypr-myproject
cd calypr-myproject
git-drs init calypr-myproject
```
...
* Within the project, there are a couple important directories...
    ...
    * `.drs/`: hidden, stores and manages git-drs state for the project
...
Once your project is initialized, you can add files to the project's manifest. For example, let's say you have tsv files in a `folder/` directory within your current repository. Each of the tsv files are associated with a particular subject, say `patient_1` and `patient_2`. To add them using `git-drs add`,

```bash
git-drs add folder/file.tsv --patient patient_1
git-drs add folder/file2.tsv --patient patient_2
```

* Each `git-drs add` above creates a metadata entry for the specified data file, automatically calculating metadata like the file's md5sum, type, date modified, size, and path.
...
* `git-drs add` varies from `git add`, as the `.dvc` file is what gets staged rather than the potentially large data file
* Multiple files can be added at the same time by wrapping a wildcard string in quotes, for example, `git-drs add "*.csv"`.
...
Now that your files have been staged with metadata entries, you can create FHIR-compliant metadata using the `git-drs meta init` command

```bash
git-drs meta init
```

* Using the file metadata entries created by the `git-drs add` command, `git-drs meta init` creates FHIR-compliant metadata files in the `META/` directory, where each file corresponds to a [FHIR resource](https://build.fhir.org/resourcelist.html). At a minimum, the directory will contain:
...
- `meta init` is a good example of where git-drs differs from git! While you might go from `git add` straight to `git commit` in a git workflow, we have to do `git-drs add` > `git-drs meta init` > `git-drs commit` to track both the files and each file's metadata in git-drs.
...
To ensure that the FHIR data has been properly formatted, you can call `git-drs meta validate`.
...
You can double-check that all of your files have been staged with `git-drs status`
...
With all checks complete, you can commit the metadata we created using `git-drs commit`.

```bash
git-drs commit -m "adding tsv metadata"
```
...
To submit the files and metadata to the data platform, we can use `git-drs push`
...
Sometimes you might want the most recent version of a data project that has already been published to the platform. To download the metadata for an existing project, use the `git-drs clone` command.
